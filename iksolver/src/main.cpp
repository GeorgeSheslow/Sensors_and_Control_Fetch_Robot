...
{
  const std::string node_name = "iksolver";
  ...
  // Using: moveit_core:`RobotModel`, and planning_scene: `PlanningScene`, creates a planning scene (including the robot).
  planning_scene::PlanningScenePtr planning_scene(new planning_scene::PlanningScene(robot_model));

  // Configure a valid robot state
  planning_scene->getCurrentStateNonConst().setToDefaultValues(joint_model_group, "ready");

  // We will now construct a loader to load a planner, by name. Using the ROS pluginlib library.
  boost::scoped_ptr<pluginlib::ClassLoader<planning_interface::PlannerManager>> planner_plugin_loader;
  planning_interface::PlannerManagerPtr planner_instance;
  std::string planner_plugin_name;

  // We will get the name of planning plugin we want to load from the ROS parameter server, and then load the planner making sure to catch all exceptions.
  ...
  // Visualization
  // ^^^^^^^^^^^^^
  // MoveItVisualTools provides many capabilities for visualizing objects, robots, and trajectories in RViz as well as debugging tools such as step-by-step introspection of a script.
  ...
  /*Visual_tools to wait for user input */
  visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to start the demo");

  // Pose Goal
  // ^^^^^^^^^
  // Motion plan request for the arm of the Panda specifying the desired pose of the end-effector as input.
  ...
  /* But, let's impose a path constraint on the motion. Asking for the end-effector to stay level*/
  ...
  // Imposing path constraints requires the planner to reason in the space of possible positions of the end-effector (the workspace of the robot)
